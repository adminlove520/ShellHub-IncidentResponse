...
echo -e ""
echo -e "  =========================================================  "
echo -e "            Linux应急处置/信息搜集/漏洞检测脚本 V3.2            "
echo -e "  =========================================================  "
echo -e "  支持Centos、Debian系统检测                                   "
echo -e "  author： al0ne Mannix                               "
echo -e "  https://github.com/al0ne                                   "
echo -e "  更新日期：2022年10月23日                                     "
echo -e "  参考来源：                                                  "
echo -e "    1.Gscan  https://github.com/grayddq/GScan                "
echo -e "    2.Lynis  https://github.com/CISOfy/lynis                 "
echo -e "    3.LinuxCheck  https://github.com/al0ne                   "
echo -e "\n"
...
for prog in "${cmdline[@]}"; do
  ...
  if [ $OS = 'Centos' ]; then
    soft=$(rpm -q "$prog")
    if echo -e "$soft" | grep -E '没有安装|未安装|not installed' >/dev/null 2>&1; then
      echo -e "$prog 安装中......"
...
base_check() {
  ...
  #操作系统信息
  echo -e "Operating System:\t\t$(uname -a)" 2>/dev/null | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
  echo -e "Operating System:\t\t$(cat /proc/version)" 2>/dev/null | tee -a "$filename"
  ...
  #cpu信息lscpu
  echo -e "CPU info:\n$(lscpu)" 2>/dev/null | tee -a "$filename"
  ...
  awk '$0 ~/cpu[0-9]/' /proc/stat 2>/dev/null | while read line; do
    echo -e "$line" | awk '{total=$2+$3+$4+$5+$6+$7+$8;free=$5;\
  ...
  #登录账号
  echo -e "\e[00;31m[+]登录账号\e[00m" | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
  who | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
...
network_check() {
  ...
  echo -e "\n" | tee -a "$filename"
  echo -e "Interface    ByteRec   PackRec   ByteTran   PackTran" | tee -a "$filename"
...
crontab_check() {
  ...
  echo -e "\n" | tee -a "$filename"
  ls -laht /etc/cron.*/* | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
...
user_check() {
  ...
  echo -e "\e[00;31m[+]可登录账号\e[00m" | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
  cat /etc/passwd | ag '/bin/bash$' | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"

  echo -e "\e[00;31m[+]最近20条登录失败信息\e[00m" | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
  lastb | head -n 20 | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"

  echo -e "\e[00;31m[+]空口令账号\e[00m" | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
  awk -F: '{if($2==0)print $1}' /etc/shadow | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
  ...
  echo -e "登录ip: $(ag -a accepted /var/log/secure /var/log/auth.* 2>/dev/null | ag -o '\d+\.\d+\.\d+\.\d+' | sort | uniq)" | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
}
...
service_check() {
  ...
  echo -e "\e[00;31m[+]系统运行服务 \e[00m" | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
  chkconfig --list | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
  systemctl list-unit-files | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"

  echo -e "\e[00;31m[+]所有服务状态 \e[00m" | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
  service --status-all | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
  systemctl list-units --type service --all | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"

  echo -e "\e[00;31m[+]最近添加的Service \e[00m" | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
  ls -lahtR /etc/systemd/system/multi-user.target.wants | tee -a "$filename"
  ls -laht /etc/systemd/system/*.service | ag -v 'dbus-org' | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
}
...
bash_check() {
  ...
  echo -e "\n" | tee -a "$filename"
  ls -laht /root/.*_history | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
...
file_check() {
  ...
  echo -e "\e[00;31m[+]系统文件修改时间 \e[00m" | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
  cmdline=(
    "/sbin/ifconfig"
    "/bin/ls"
  ...
  for soft in "${cmdline[@]}"; do
    echo -e "文件：$soft\t\t\t修改日期：$(stat $soft | ag -o '(?<=Modify: )[\d-\s:]+')" | tee -a "$filename"
  done
  echo -e "\n" | tee -a "$filename"
  ...
  echo -e "\e[00;31m[+]查看隐藏进程 \e[00m" | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
  ps -ef | awk '{print}' | sort -n | uniq > 1 | tee -a "$filename"
  ls /proc | sort -n | uniq > 2 | tee -a "$filename"
  diff 1 2 | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
  ...
  echo -e "\n" | tee -a "$filename"
  find /etc /bin /lib /sbin /dev /root/ /home /tmp /var /usr ! -path "/var/log*" ! -path "/var/spool/exim4*" ! -path "/var/backups*" -mtime -7 -type f | ag -v '\.log|cache|vim|/share/|/lib/|.zsh|.gem|\.git|LICENSE|README|/_\w+\.\w+|\blogs\b|elasticsearch|nohup|i18n' | xargs -i{} ls -lah {} | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"

  #近7天改动
  echo -e "\e[00;31m[+]近七天文件改动 ctime \e[00m" | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
  find /etc /bin /lib /sbin /dev /root/ /home /tmp /var /usr ! -path "/var/log*" ! -path "/var/spool/exim4*" ! -path "/var/backups*" -ctime -7 -type f | ag -v '\.log|cache|vim|/share/|/lib/|.zsh|.gem|\.git|LICENSE|README|/_\w+\.\w+|\blogs\b|elasticsearch|nohup|i18n' | xargs -i{} ls -lah {} | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
  ...
  echo -e "\n" | tee -a "$filename"
  find / ! -path "/proc/*" ! -path "/sys/*" ! -path "/run/*" ! -path "/boot/*" -size +200M -exec ls -laht {} + 2>/dev/null | ag '\.gif|\.jpeg|\.jpg|\.png|\.zip|\.tar.gz|\.tgz|\.7z|\.log|\.xz|\.rar|\.bak|\.old|\.sql|\.1|\.txt|\.tar|\.db|/\w+$' --nocolor | ag -v 'ib_logfile|ibd|mysql-bin|mysql-slow|ibdata1' | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"

  #敏感文件
  echo -e "\e[00;31m[+]敏感文件 \e[00m" | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
  find / ! -path "/lib/modules*" ! -path "/usr/src*" ! -path "/snap*" ! -path "/usr/include/*" -regextype posix-extended -regex '.*sqlmap|.*msfconsole|.*\bncat|.*\bnmap|.*nikto|.*ettercap|.*tunnel\.(php|jsp|asp|py)|.*/nc\b|.*socks.(php|jsp|asp|py)|.*proxy.(php|jsp|asp|py)|.*brook.*|.*frps|.*frpc|.*aircrack|.*hydra|.*miner|.*/ew$' -type f | ag -v '/lib/python' | xargs -i{} ls -lah {} | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"

  echo -e "\e[00;31m[+]可疑黑客文件 \e[00m" | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
  find /root /home /opt /tmp /var/ /dev -regextype posix-extended -regex '.*wget|.*curl|.*openssl|.*mysql' -type f 2>/dev/null | xargs -i{} ls -lah {} | ag -v '/pkgs/|/envs/' | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
}
...
rootkit_check() {
  ...
  if [ -n "$kernel" ]; then
    ...
    echo -e "$kernel" | tee -a "$filename"
  else
    echo -e "未找到内核敏感函数" | tee -a "$filename"
  fi
  echo -e "\n" | tee -a "$filename"

  echo -e "\e[00;31m[+]可疑的.ko模块\e[00m" | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
  find / ! -path "/proc/*" ! -path "/usr/lib/modules/*" ! -path "/lib/modules/*" ! -path "/boot/*" -regextype posix-extended -regex '.*\.ko' | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
}
...
ssh_check() {
  ...
  #ssh后门配置检查
  echo -e "\e[00;31m[+]SSH 后门配置 \e[00m" | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
  if [ -e "$HOME/.ssh/config" ]; then
    grep LocalCommand <~/.ssh/config | tee -a "$filename"
    grep ProxyCommand <~/.ssh/config | tee -a "$filename"
  else
    echo -e "未发现ssh配置文件" | tee -a "$filename"
  fi
  echo -e "\n" | tee -a "$filename"

  #ssh后门配置检查
  echo -e "\e[00;31m[+]SSH 软连接后门 \e[00m" | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
  if ps -ef | ag '\s+\-oport=\d+' >/dev/null 2>&1; then
    ps -ef | ag '\s+\-oport=\d+' | tee -a "$filename"
  else
    echo -e "未检测到SSH软连接后门" | tee -a "$filename"

  fi
  echo -e "\n" | tee -a "$filename"

  echo -e "\e[00;31m[+]SSH inetd后门检查 \e[00m" | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
  if [ -e "/etc/inetd.conf" ]; then
    grep -E '(bash -i)' </etc/inetd.conf | tee -a "$filename"
  fi
  echo -e "\n" | tee -a "$filename"
...
poison_check() {
  ...
  echo -e "\e[00;31m[+]Python2 pip 检测\e[00m" | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
  pip freeze | ag "istrib|djanga|easyinstall|junkeldat|libpeshka|mumpy|mybiubiubiu|nmap-python|openvc|python-ftp|pythonkafka|python-mongo|python-mysql|python-mysqldb|python-openssl|python-sqlite|virtualnv|mateplotlib|request=" | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"

  echo -e "\e[00;31m[+]Python3 pip 检测\e[00m" | tee -a "$filename"
  pip3 freeze | ag "istrib|djanga|easyinstall|junkeldat|libpeshka|mumpy|mybiubiubiu|nmap-python|openvc|python-ftp|pythonkafka|python-mongo|python-mysql|python-mysqldb|python-openssl|python-sqlite|virtualnv|mateplotlib|request=" | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
}
...
risk_check() {
  ...
  echo -e "\e[00;31m[+]Redis弱密码检测\e[00m" | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
  cat /etc/redis/redis.conf 2>/dev/null | ag '(?<=requirepass )(test|123456|admin|root|12345678|111111|p@ssw0rd|test|qwerty|zxcvbnm|123123|12344321|123qwe|password|1qaz|000000|666666|888888)' | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
}
